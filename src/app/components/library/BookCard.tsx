...
>; // Add or remove fields as needed for the card display. 'id' here will be user_books.id

const BookCard = ({ id, title, author, genre, readingProgress, coverImageUrl, isPinned }: BookCardProps) => {
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 ease-in-out h-full flex flex-col group">
      {coverImageUrl ? (
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-auto pt-2 flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0">
          <Link 
            href={`/library/${id}`}
            className="w-full sm:w-auto flex-grow text-center px-4 py-2 text-sm font-medium text-indigo-600 bg-indigo-100 rounded-md hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors"
          >
            Details
          </Link>
          <Link 
            href={`/reader/${id}`}
            className="w-full sm:w-auto flex-grow text-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors"
          >
            Continue Reading
          </Link>
        </div>
