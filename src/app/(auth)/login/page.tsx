...
export default function LoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (signInError) {
      ...
      // On successful login, Supabase client handles session. Redirect to dashboard or desired page.
      // The AuthProvider (to be created next) will listen to auth state changes and update UI.
  ...
  const handleOAuthSignIn = async (provider: 'google') => {
    ...
    console.log('OAuth sign in response:', { data, oauthError });

    if (oauthError) {
      setError(`Error signing in with ${provider}: ${oauthError.message}`);
    } else {
      // Redirect usually happens automatically.
